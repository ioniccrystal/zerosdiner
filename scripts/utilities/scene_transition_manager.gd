...
func change_scene_with_transition(scene: PackedScene, transition: PackedScene):
	var transition_instance: SceneTransition = transition.instantiate()
...
func _on_fade_out(new_scene: PackedScene, transition_instance: SceneTransition):
	change_scene(new_scene)
	transition_instance.fade_in()

func _on_fade_in(transition_instance: SceneTransition):
